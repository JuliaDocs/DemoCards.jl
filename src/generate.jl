...
function get_build_file(source_file, source, destination, build)
    # NOTE: there's some dirty resolving work here, might need a clean rewrite
    source_root = splitdir(source)[1]

    # destination and build should be consistant to what users pass to Documenter
    build_root = joinpath(build, destination)
    if isempty(source_root)
        # reached here if page source is a folder in "docs", e.g., "docs/quickstart"

        # get the parent dir path of original demo pages
        # e.g., if demo page is placed in docs/quickstart then it should be docs/
        source_root = first(split(source_file, source))

        # get the parent dir path of build page generated by Documenter
        # e.g., docs/build/democards
        build_root = joinpath(source_root, build_root)
    end

    # resolve the original file path from generated file path
