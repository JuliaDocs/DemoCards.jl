...
* (Optional) `templates`: card theme generated by [`cardtheme`](@ref). If it is not passed, it would
  not generate the index page for your demos, and would use Documenter's sidebar for navigation.
...
The following is a themeless example for you to start, in this case, it won't generate index page
for your demos, and would use Documenter's sidebar for navigation purpose.
...
To generate an index page and associated cover images for better navigation, you could optionally
pass `templates` and `assets`, which is generated from [`cardtheme`](@ref):
